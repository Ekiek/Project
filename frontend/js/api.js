...
export async function getHotels(city = "") {
  let url = `${API_BASE_URL}/hotels/`;
  if (city) url += `?city=${encodeURIComponent(city)}`;
  return await fetchJSON(url, { headers: getAuthHeaders(false) });
}
...
// --- Book Room
export async function bookRoom(room, guest_name, guest_phone, check_in, check_out) {
  return await fetchJSON(`${API_BASE_URL}/bookings/`, {
    method: "POST",
...
// --- Cancel Booking
export async function cancelBooking(id) {
  return await fetchJSON(`${API_BASE_URL}/bookings/${id}/cancel/`, {
    method: "POST",
    headers: getAuthHeaders()
  });
}
...
// --- Update Booking
export async function updateBooking(id, data) {
  return await fetchJSON(`${API_BASE_URL}/bookings/${id}/`, {
    method: "PATCH",
    headers: getAuthHeaders(true),
    body: JSON.stringify(data)
  });
...
export async function getCurrentUser() {
  return await fetchJSON(`${API_BASE_URL}/me/`, { headers: getAuthHeaders() });
}
